...
Last modified: 7/11/2021 11:03:39 PM
"""
...
#            if (True):
            ...
            #i_p = 100
            #s = np.arange(k1, l)
            #u = np.linspace(k1,l,(l-k1)*i_p)
            #ch_slice = np.array(y[ymaxpos+k1:ymaxpos+l])
            #chsinc = sinc_interp(ch_slice, s, u)
            #print (np.max(y) - int(np.mean(y[ymaxpos-100:ymaxpos-50])), np.max(chsinc) - int(np.mean(y[ymaxpos-100:ymaxpos-50])))
            #exit()
            ...
            #    period = y_oft2 - y_oft1 + period
            #    y_oft = y_oft1 + period
            #    #print (period, y_oft2, y_oft1)
            #print (y_oft, np.max(y[period:period*2]))
            y_plt_oft = y_oft-50
            y = y[y_plt_oft:]
            leny = len(y)
            ...
            pltlen =100 
            x = np.arange(pltlen)
            #x = np.linspace(k1,l,101)
            #print (x)
            #print (len(x))
            plt.plot(x*0.5,ay[50 - (pltlen//2):50+(pltlen//2)] , marker='.', label="ASIC%02d"%k, color = "C{}".format(k))
            #plt.plot(u, chsinc)
            #plt.scatter(x[0:100], y[ymaxpos-(pltlen//2):ymaxpos+(pltlen//2)], marker = '.')
            plt.grid(True)
            plt.ylim((0,4000))
...
#            plt.close()
    ...
    plt.show()
#    plt.savefig()
#    plt.savefig(frst + fp[0:-3] + "chk_averaging_wfm.png")
    plt.close()
